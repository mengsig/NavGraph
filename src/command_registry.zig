...
pub const command_descriptors = [_]CommandDescriptor{
    ...
    .{ .command = .events, .name = "events", .summary = "Link message-bus event handlers to their emitters.", .example = "navgraph events buildOpenDir", .aliases = &.{ "dispatch", "bus" }, .arguments = &optional_filter, .options = &.{ .root, .no_cache, .backend, .limit, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .catches, .name = "catches", .summary = "Show what handles a given exception type, and what does not.", .example = "navgraph catches load --strict", .aliases = &.{"handles"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .limit, .strict, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .flow, .name = "flow", .summary = "Show where a value is written and where it's read.", .example = "navgraph flow buildOpenDir --to reload", .aliases = &.{"dataflow"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .limit, .strict, .format, .writers, .readers, .unread, .on_type, .to }, .conflicts = &flow_conflicts, .outputs = &text_json, .access = .read_only, .requires_index = true },
