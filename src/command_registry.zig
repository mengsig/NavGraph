...
pub const CommandDescriptor = struct {
    command: Command,
    name: []const u8,
    /// One plain-English line for `navgraph help <command>` and the top-level
    /// catalogue — no internal terms (resolver, backend tier, ...).
    summary: []const u8,
...
pub const command_descriptors = [_]CommandDescriptor{
    ...
    .{ .command = .docs, .name = "docs", .summary = "Show the docstring or leading comment for a definition.", .example = "navgraph docs FeasibilityProvider.check", .aliases = &.{"doc"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .events, .name = "events", .summary = "Link message-bus event handlers to their emitters.", .example = "navgraph events order_placed", .aliases = &.{ "dispatch", "bus" }, .arguments = &optional_filter, .options = &.{ .root, .no_cache, .backend, .limit, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .hierarchy, .name = "hierarchy", .summary = "Show a type's supertypes and subtypes (its inheritance tree).", .example = "navgraph hierarchy Leaf --overrides", .aliases = &.{"hier"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .limit, .strict, .format, .overrides }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    .{ .command = .raises, .name = "raises", .summary = "Trace where a symbol's exceptions are caught, or go uncaught.", .example = "navgraph raises submit_order -d 3", .aliases = &.{"throws"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .depth, .limit, .strict, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    .{ .command = .catches, .name = "catches", .summary = "Show what handles a given exception type, and what does not.", .example = "navgraph catches OrderError --strict", .aliases = &.{"handles"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .limit, .strict, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .flow, .name = "flow", .summary = "Show where a value is written and where it's read.", .example = "navgraph flow Record.value --to serialize", .aliases = &.{"dataflow"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .limit, .strict, .format, .writers, .readers, .unread, .on_type, .to }, .conflicts = &flow_conflicts, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .history, .name = "history", .summary = "Show a symbol's git history.", .example = "navgraph history negotiate --last 10", .aliases = &.{"hist"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .limit, .format, .last }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    .{ .command = .blame, .name = "blame", .summary = "Show per-line author and commit for a symbol's current code.", .example = "navgraph blame negotiate", .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .verbosity, .limit, .format }, .outputs = &text_json, .access = .read_only, .requires_index = true },
    ...
    .{ .command = .edits, .name = "edits", .summary = "List the exact source sites a rename would touch.", .example = "navgraph edits FeasibilityProvider.check", .aliases = &.{"edit-sites"}, .arguments = &symbol_arg, .options = &.{ .root, .no_cache, .backend, .limit, .format, .after }, .dependencies = &jsonl_after_dependency, .outputs = &text_json_jsonl, .access = .read_only, .requires_index = true },
